...
ENTRYPOINT service php8.1-fpm start \
    ...
    && php -r "file_put_contents('/etc/mysql/mariadb.conf.d/50-server.cnf', str_replace('bind_address', '#bind_address', file_get_contents('/etc/mysql/mariadb.conf.d/50-server.cnf')));" \
    ...
    && sudo -i mysql -uroot -proot -f /www/docker_initial.sql \
    && rm -rf /temp \
